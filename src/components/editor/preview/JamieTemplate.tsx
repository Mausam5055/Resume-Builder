      ...
      >
        <div className="flex flex-col lg:flex-row min-h-full">
          ...
          <div className="w-full lg:w-[75mm] bg-slate-50 p-4 lg:p-6 flex flex-col">
            ...
            {personal?.avatarUrl && (
              <div className="mb-6 lg:mb-8 flex justify-center">
                <Avatar className="w-32 h-32 lg:w-44 lg:h-44 border-4 border-white shadow-lg">
                  <AvatarImage 
                    src={personal.avatarUrl} 
                    alt={personal?.fullName || ''} 
                    className="object-cover"
                  />
                  <AvatarFallback className="text-2xl lg:text-4xl bg-slate-200 text-slate-600">
            ...
            <section className="mb-6 lg:mb-8">
              <h2 className="text-base lg:text-lg font-bold mb-3 lg:mb-4 text-slate-800 uppercase border-b-2 border-slate-200 pb-2">
                CONTACT
              </h2>
              <div className="space-y-2 lg:space-y-3">
                {personal?.phone && (
                  <div className="flex items-start gap-2 lg:gap-3 text-slate-700">
                ...
                <div className="space-y-3 lg:space-y-4">
                  {skills.map((skillGroup) => (
                    <div key={skillGroup.id}>
                      <h3 className="font-semibold text-xs lg:text-sm text-slate-700 mb-2">{skillGroup.name}</h3>
                      <ul className="list-none space-y-1">
                        {skillGroup.skills.map((skill) => (
                          <li key={skill.id} className="text-xs lg:text-sm text-slate-600 flex items-center">
                            <span className="w-1.5 h-1.5 lg:w-2 lg:h-2 bg-slate-400 rounded-full mr-2 flex-shrink-0"></span>
                            {skill.name}
                          </li>
                        ))}
                      </ul>
          ...
          <div className="flex-1 bg-white flex flex-col">
            {/* Header */}
            <div className="bg-slate-800 text-white px-4 lg:px-8 py-6 lg:py-8 flex-shrink-0">
              <h1 className="text-2xl lg:text-4xl font-bold tracking-tight break-words">
                {personal?.fullName || 'FULL NAME'}
              </h1>
              <h2 className="text-lg lg:text-2xl mt-2 text-slate-200 break-words">
            ...
            <div className="relative p-4 lg:p-8 flex-1">
              ...
              <div className="hidden lg:block absolute left-[28px] top-0 bottom-0 w-[2px] bg-slate-200" />
              ...
              <div className="space-y-6 lg:space-y-8">
                ...
                {enabledSections.find(s => s.id === 'summary') && summary?.text && (
                  <section className="relative">
                    <div className="flex items-center mb-3 lg:mb-4">
                      <div className="hidden lg:flex w-12 h-12 lg:w-14 lg:h-14 rounded-full bg-slate-800 items-center justify-center text-white z-10 flex-shrink-0">
                        <User className="w-5 h-5 lg:w-7 lg:h-7" />
                      </div>
                      <h2 className="text-lg lg:text-xl font-bold uppercase lg:ml-5 text-slate-800">
                        PROFILE
                      </h2>
                    </div>
                ...
                {enabledSections.find(s => s.id === 'experience') && experience?.length > 0 && (
                  <section className="relative">
                    ...
                    <div className="lg:ml-[72px] space-y-4 lg:space-y-6">
                      {experience.map((exp) => (
                        <div key={exp.id} className="relative">
                          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-2 gap-2">
                            <div className="flex-1 min-w-0">
                              <h3 className="font-bold text-base lg:text-lg text-slate-800 break-words">{exp.company}</h3>
                          ...
                          {exp.description && (
                            <p className="text-xs lg:text-sm text-slate-600 mb-2">{exp.description}</p>
                          )}
                          {exp.bullets && exp.bullets.length > 0 && exp.bullets.some(bullet => bullet.trim()) && (
                            <ul className="list-disc ml-4 space-y-1">
                              {exp.bullets.filter(bullet => bullet.trim()).map((bullet, idx) => (
                                <li key={idx} className="text-xs lg:text-base text-slate-600">
                                  {bullet}
                                </li>
                              ))}
                            </ul>
                      ...
                      {education.map((edu) => (
                        <div key={edu.id}>
                          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-2">
                            <div className="flex-1 min-w-0">
                              <h3 className="font-bold text-base lg:text-lg text-slate-800 break-words">{edu.degree}</h3>
                          ...
                          {edu.description && (
                            <p className="text-xs lg:text-sm text-slate-600 mt-2">{edu.description}</p>
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  <section className="relative">
                    ...
                    <div className="lg:ml-[72px] space-y-4 lg:space-y-6">
                      {projects.map((project) => (
                        <div key={project.id}>
                          <h3 className="font-bold text-base lg:text-lg text-slate-800 break-words">{project.name}</h3>
                          <p className="text-sm lg:text-base text-slate-600 mb-2">{project.description}</p>
